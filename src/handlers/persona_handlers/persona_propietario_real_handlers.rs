...
use crate::{
    ...
    validators::persona_validators::persona_propietario_real_validators::validar_nueva_persona_propietario_real,
    AppState,
};

pub async fn crear_nueva_persona_propietario_real_handler(
    State(data): State<Arc<AppState>>,
    Path(params): Path<ObtenerPersonaParams>,
    Json(body): Json<CrearPersonaPropietarioRealSchema>,
) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
    validar_nueva_persona_propietario_real(&data, params.id_persona, &body).await?;
    let nuevo_proveedor = sqlx::query_as!(
        PropietarioRealPersonaModelo,
        r#"INSERT INTO propietarios_reales_persona
        ...
        RETURNING
        id_persona_propietario_real, id_persona, 
...
pub async fn obtener_persona_propietario_real_handler(
    data: &Arc<AppState>,
    id_persona: i32,
) -> Result<PropietarioRealPersonaModelo, (StatusCode, Json<serde_json::Value>)> {
    let propietario_encontrado = sqlx::query_as!(
        PropietarioRealPersonaModelo,
        r#"SELECT id_persona_propietario_real, id_persona, 
        clasificacion AS "clasificacion: ClasificacionPersona",
        ocupacion_pld, especificacion_pld, antiguedad, actividad_pld,
        periodo AS "periodo: PeriodoPersona", frecuencia_captacion, operacion_maxima_captacion,
        perfil_frecuencia_prestamo, operacion_maxima_prestamo,
        ...
        regimen_conyugal AS "regimen_conyugal: RegimenConyugalPersona",profesion,escolaridad, 
        autorizo_compartir_informacion_ifai,autorizo_publicidad 
        FROM propietarios_reales_persona
        WHERE id_persona=$1"#,
        id_persona,
    )
    .fetch_one(&data.db)
